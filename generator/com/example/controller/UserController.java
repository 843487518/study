...
public class UserController {
		 ...
		 public ApiResult getDetail(@PathVariable("id") long id){
		        ...
		        tbUserDTO.setId(id); 
		        return ApiResult.success(userService.query(tbUserDTO));
		 }
		 @PostMapping("/list")
		 public ApiResult getList(@RequestBody UserInput userInput){
		        ...
		        return ApiResult.success(userService.queryList(tbUserDTO));
		 }
		 @PostMapping("/add")
		 ...
		 public ApiResult modify(@RequestBody UserInput userInput){
		        ...
		        BeanUtils.copyProperties(userInput, tbUserDTO);
		        //TODO 如果有日期类型，记得要转换一下！！！
		        tbUserDTO.setUpdateTime(DateUtils.getDateFromFormat(input.getUpdateTime()));
